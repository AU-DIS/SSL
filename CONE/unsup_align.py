...
def objective(X, Y, R, n=5):
    ...
    P = ot.sinkhorn(np.ones(n), np.ones(n), np.asarray(C), 0.025, stopThr=1e-3)
...
def align(X, Y, R, lr=1.0, bsz=10, nepoch=5, niter=10,
    ...
    for epoch in range(1, nepoch + 1):
        for _it in range(1, niter + 1):
            ...
            #print bsz, C.shape
            P = ot.sinkhorn(np.ones(bsz), np.ones(bsz), np.asarray(C), reg, stopThr=1e-3)
...
def convex_init_sparse(X, Y, K_X = None, K_Y = None, niter=10, reg=1.0, apply_sqrt=False, P = None):
    ...
    for it in range(1, niter + 1):
        ...
        q = ot.sinkhorn(np.ones(n), np.ones(n), np.asarray(G), reg, stopThr=1e-3)
